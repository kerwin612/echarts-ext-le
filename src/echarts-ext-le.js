	...
	function LeECharts (chart, theme, opts) {
		...
		LeEChartsProto.getDupECharts = function () {
			...
			for (var i = 0, j = arguments.length; i < j && i < 2; i++) {
				if (!dom && arguments[i] instanceof HTMLElement) dom = arguments[i];
				if (!option && !(arguments[i] instanceof HTMLElement)) option = arguments[i];
			}
		...
		LeEChartsProto.downloadChart = function () {
			var name, option;
			for (var i = 0, j = arguments.length; i < j && i < 2; i++) {
				if (!name && typeof(arguments[i]) === 'string') name = arguments[i];
				if (!option && typeof(arguments[i]) === 'object') option = arguments[i];
			}
